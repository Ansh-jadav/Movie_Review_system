  /**
     * Critic's Cut — Vanilla JS app
     * Data sources:
     *  - OMDb: basic details (title/year/poster/plot/genre/rated/runtime)
     *  - TMDb: video clips (trailer, interviews, bloopers, etc.)
     * Storage: localStorage per imdbID
     */

    // ⛳ Replace with your keys
    const OMDB_KEY = "9e1173d1";    // https://www.omdbapi.com/apikey.aspx
    const TMDB_KEY = "03d8b7d509444728580407016080187b";    // https://www.themoviedb.org/settings/api
    ...
    const safePoster = (url) => (url && url !== 'N/A') ? url : 'data:image/svg+xml;charset=UTF-8,' + encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="400" height="600"><rect width="100%" height="100%" fill="#0c0f16"/><text x="50%" y="50%" fill="#93a3b8" dominant-baseline="middle" text-anchor="middle" font-family="Arial" font-size="20">No Poster</text></svg>`);
    ...
    clearBtn.addEventListener('click', () => { if (confirm('Clear all saved reviews on this device?')) { clearAll(); if (currentMovie) { renderReviews(currentMovie.imdbID); updateSentimentBar(currentMovie.imdbID); } }});

    // Search logic
    async function searchMovies(q) {
      if (!OMDB_KEY || OMDB_KEY === 'YOUR_OMDB_KEY') { alert('Add your OMDb API key in the source first.'); return []; }
    ...
    async function getMovieById(imdbID) {
      const res = await fetch(`https://www.omdbapi.com/?apikey=${OMDB_KEY}&i=${imdbID}&plot=full`);
    ...
    // TMDb helpers for videos
    async function tmdbFindByIMDb(imdbID) {
      if (!TMDB_KEY || TMDB_KEY === 'YOUR_TMDB_KEY') { return null; }
      const url = `https://api.themoviedb.org/3/find/${imdbID}?api_key=${TMDB_KEY}&external_source=imdb_id`;
      const res = await fetch(url);
      if (!res.ok) return null;
      const data = await res.json();
      const match = (data.movie_results && data.movie_results[0]) || null;
      return match; // {id, title, ...}
    }

    async function tmdbVideos(tmdbId) {
      const url = `https://api.themoviedb.org/3/movie/${tmdbId}/videos?api_key=${TMDB_KEY}&language=en-US`;
      const res = await fetch(url);
      if (!res.ok) return [];
      const data = await res.json();
    ...
    function renderReviews(imdbID) {
      const arr = getReviews(imdbID);
      reviewsEl.innerHTML = '';
      if (!arr.length) { reviewsEl.appendChild(h('div', { class: 'empty' }, 'No reviews yet. Be the first critic!')); return; }
      arr.forEach(r => {
  ...
  try {
    const url = `https://www.omdbapi.com/?apikey=${OMDB_KEY}&s=${encodeURIComponent(genre)}&type=movie`;
    const res = await fetch(url);
    const data = await res.json();
    if (data.Response === 'True' && Array.isArray(data.Search)) {
      const picks = data.Search.slice(0, 5); // 1 featured + 4 related
